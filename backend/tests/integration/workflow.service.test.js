...
const emitCompletedStream = (stream, data) => {
  stream.emit(
    "data",
    Buffer.from(`event: completed\ndata: ${JSON.stringify(data)}\n\n`),
  );
  stream.emit("end");
};

const waitForAsyncListeners = () =>
...
describe("WorkflowService", () => {
  ...
  test("generateAutomation rejects non testcase source tasks", async () => {
    ...
    await expect(
      WorkflowService.generateAutomation({
        taskId: "task-source",
        framework: "appium",
      }),
  ...
  test("getLatestCompletedTask checks access on the document project", async () => {
    ...
    expect(Task.findLatestByProject).toHaveBeenCalledWith(
      "project-1",
      "extract-flows",
      "completed",
      "committed",
  ...
  test("agent 2 preserves scenarios when agent returns duplicate scenario IDs", async () => {
      ...
      .mockResolvedValueOnce([
        {
          ...
          artifactType: "scenario",
          contentJson: { id: "TC_001", flow_name: "Login" },
        },
        {
          id: "scenario-artifact-2",
          artifactType: "scenario",
          contentJson: { id: "TC_001", flow_name: "Register" },
        },
      ]);
    ...
    const scenarioRecords = AgentArtifact.bulkUpsert.mock.calls
      .map(([records]) => records)
      .find((records) =>
        records?.every((record) => record.artifactType === "scenario")
      );
    expect(scenarioRecords).toHaveLength(2);
    expect(scenarioRecords.map((record) => record.contentJson.flow_name))
      .toEqual(["Login", "Register"]);
    expect(new Set(scenarioRecords.map((record) => record.artifactKey)).size)
      .toBe(2);
  });
  ...
  test("agent 3 preserves YAML files when agent returns duplicate filenames", async () => {
      ...
      .mockResolvedValueOnce([
        {
          id: "yaml-artifact-1",
          artifactType: "yaml",
          contentJson: { filename: "TC_001.yaml" },
          contentText: "name: login",
        },
        {
          id: "yaml-artifact-2",
          artifactType: "yaml",
          contentJson: { filename: "TC_001.yaml" },
          contentText: "name: register",
        },
      ]);
    ...
    const yamlRecords = AgentArtifact.bulkUpsert.mock.calls
      .map(([records]) => records)
      .find((records) =>
        records?.every((record) => record.artifactType === "yaml")
      );
    expect(yamlRecords).toHaveLength(2);
    expect(yamlRecords.map((record) => record.contentText)).toEqual([
      "name: login",
      "name: register",
    ]);
    expect(new Set(yamlRecords.map((record) => record.artifactKey)).size).toBe(
      2,
    );
  });
});
